...
export default function CreateServicePage() {
  ...
  const [selectedTags, setSelectedTags] = useState<string[]>([]); // ADD THIS LINE
  ...
  // Toggle tag selection - ADD THIS FUNCTION
  ...
  // Validate form - UPDATE THIS FUNCTION
  const validateForm = () => {
    ...
    if (formData.description.length < 50) newErrors.description = 'Please provide a more detailed description (min 50 characters)';
    if (formData.description.length > 1000) newErrors.description = 'Description is too long (max 1000 characters)';
    
    if (!formData.price.trim()) newErrors.price = 'Price is required';
    if (isNaN(Number(formData.price)) || Number(formData.price) <= 0) newErrors.price = 'Please enter a valid price';

    // UPDATE THIS VALIDATION - use selectedTags instead of formData.tags
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full rounded-xl border ${errors.title ? 'border-red-500' : 'border-zinc-300 dark:border-zinc-700'} bg-white dark:bg-zinc-800 px-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent`}
                  />
                  {errors.title && (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full rounded-xl border ${errors.description ? 'border-red-500' : 'border-zinc-300 dark:border-zinc-700'} bg-white dark:bg-zinc-800 px-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent`}
                  />
                  {errors.description && (
                    <p className="mt-2 text-sm text-red-500 flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="relative">
                      <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 text-zinc-500" size={20} />
                      <input
                        type="text"
                        name="price"
                        ...
                        onChange={handleChange}
                        placeholder="50.00"
                        className={`w-full rounded-xl border ${errors.price ? 'border-red-500' : 'border-zinc-300 dark:border-zinc-700'} bg-white dark:bg-zinc-800 pl-10 pr-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent`}
                      />
                    </div>
                    {errors.price && (
                      <p className="mt-2 text-sm text-red-500 flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {categories.map((cat) => (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center p-3 rounded-xl border ${formData.category === cat.id ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-zinc-300 dark:border-zinc-700'} transition-colors`}
                        >
                          {cat.icon}
                ...
                <div>
                  ...
                  <div className="space-y-2.5">
                    {/* Content Rating */}
                    <div>
                      <h4 className="text-sm font-medium mb-1.5 text-zinc-700 dark:text-zinc-300">Content Rating</h4>
                      <div className="flex flex-wrap gap-1.5">
                        {['NSFW', 'SFW'].map((tag) => (
                          <button
                            ...
                            className={`px-2.5 py-1 rounded-full border text-xs ${selectedTags.includes(tag) ? 'bg-purple-100 border-purple-500 text-purple-700 dark:bg-purple-900/30 dark:border-purple-700 dark:text-purple-300' : 'bg-zinc-100 border-zinc-300 text-zinc-700 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-300'} transition-colors hover:scale-115`}
                          >
                            {tag}
                          </button>
                        ))}
                    ...
                    {/* Body Type */}
                    <div>
                      <h4 className="text-sm font-medium mb-1.5 text-zinc-700 dark:text-zinc-300">Body Type</h4>
                    ...
                    {/* Platform */}
                    <div>
                      <h4 className="text-sm font-medium mb-1.5 text-zinc-700 dark:text-zinc-300">Platform</h4>
                      <div className="flex flex-wrap gap-2">
                    ...
                    {/* Content Type */}
                    <div>
                      <h4 className="text-sm font-medium mb-1.5 text-zinc-700 dark:text-zinc-300">Content Type</h4>
                      <div className="flex flex-wrap gap-2">
                    ...
                    {/* VR Platform */}
                    <div>
                      <h4 className="text-sm font-medium mb-1.5 text-zinc-700 dark:text-zinc-300">VR Platform</h4>
                      <div className="flex flex-wrap gap-2">
                        {['PC', 'Quest'].map((tag) => (
                          <button
                            key={tag}
                  ...
                  {selectedTags.length > 0 && (
                    <div className="mt-4">
                      <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-1.5">Selected tags:</p>
                      <div className="flex flex-wrap gap-2">
                        {selectedTags.map((tag) => (
                          <div key={tag} className="flex items-center gap-1 rounded-full bg-purple-100 dark:bg-purple-900/30 px-3 py-0.5">
                            <span className="text-sm text-purple-700 dark:text-purple-300">{tag}</span>
                            <button
                              type="button"
                              onClick={() => toggleTag(tag)}
                ...
                {/* Submit Errors */}
                {submitError && (
                    <div className="rounded-xl border boder-red-200 bg-red-50 p-4 text-sm text-red-700">
                        {submitError}
                    </div>
                )}
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <div className="text-zinc-600 dark:text-zinc-400">
                      Delivery: {
                      formData.deliveryTime === '1-3' ? '1 - 3 days' :
                      formData.deliveryTime === '4-6' ? '4 - 6 days' :
                      `${formData.deliveryTime} day${formData.deliveryTime !== '1' ? 's' : ''}` }
                    </div>
                  </div>
